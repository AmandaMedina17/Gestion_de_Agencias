...
async function bootstrap() {
  ...
  try {
    ...
    console.log('🏙️ Creando lugares (PlaceEntity) para cada ciudad...');
    const places: PlaceEntity[] = [];
    const cityToPlaceEntity: Map<string, PlaceEntity> = new Map();

    // Crear un PlaceEntity para cada ciudad única
    for (const city of koreanCities) {
      const placeEntity = placeRepo.create({
        id: randomUUID(),
        place: city
      });
      places.push(placeEntity);
      cityToPlaceEntity.set(city, placeEntity);
    }

    await placeRepo.save(places);
    console.log(`✅ ${places.length} lugares creados correctamente.`);
    ...
    // Crear 50 agencias
    for (let i = 0; i < 50; i++) {
      const city = faker.helpers.arrayElement(koreanCities);
      const placeEntity = cityToPlaceEntity.get(city);
      const nameAgency = agencyNames[i] || `${city} Entertainment`;
      
      agencies.push({
        id: randomUUID(),
        place: placeEntity!, // Asignar el ID del lugar
        dateFundation: faker.date.between({ 
          from: '1990-01-01', 
          to: '2020-12-31' 
        }),
    ...
    await agencyRepo.save(agencies);
    console.log("✅ 50 agencias insertadas correctamente con lugares asignados.");

    // Verificar la relación
    console.log('\n📊 Verificando relaciones agencia-lugar:');
    const sampleAgencies = await agencyRepo.find({
      take: 5,
      relations: ['place']
    });

    sampleAgencies.forEach(agency => {
      console.log(`   Agencia: ${agency.name} -> Lugar: ${agency.place?.place || 'Sin lugar'}`);
    });
    ...
    console.log('\n📊 ESTADÍSTICAS DEL SEEDING:');
    console.log(`🏙️ Lugares creados: ${places.length}`);
    console.log(`🏢 Agencias creadas: ${agencies.length}`);
    console.log(`🎓 Aprendices creados: ${apprentices.length}`);
    console.log(`🎤 Artistas creados: ${artists.length}`);
    console.log(`👤 Usuarios creados/actualizados: ${createdCount}`);
    ...
    // Mostrar distribución de agencias por ciudad
    console.log('\n🌆 Distribución de agencias por ciudad:');
    const placeDistribution = await agencyRepo
      .createQueryBuilder('agency')
      .select('place.place', 'city')
      .addSelect('COUNT(agency.id)', 'agencyCount')
      .innerJoin('place', 'place', 'agency.placeId = place.id')
      .groupBy('place.place')
      .orderBy('COUNT(agency.id)', 'DESC')
      .getRawMany();

    placeDistribution.forEach(place => {
      console.log(`   ${place.city}: ${place.agencyCount} agencias`);
    });
