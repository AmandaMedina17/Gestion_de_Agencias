...
const GroupManagement: React.FC = () => {
    ...
    const [deletingGroup, setDeletingGroup] = useState<GroupResponseDto | null>(null);
    const [viewingGroup, setViewingGroup] = useState<GroupResponseDto | null>(null);
    const [addingMemberGroup, setAddingMemberGroup] = useState<GroupResponseDto | null>(null);
    ...
    const getAgencyName = (group: GroupResponseDto) => {
        ...
        return agency ? `${agency.nameAgency} - ${agency.place}` : "Agencia no encontrada";
    };
    ...
    const groupFields: FormField[] = [
        ...
        },
        {
            ...
            validate: (value) => {
                if (value === undefined || value === null) return "Debe seleccionar una opción";
                return null;
            }
        }
    ];
    ...
    const handleCreate = async (data: Record<string, any>) => {
        try {
            if (!userAgency) {
                showError("Error", "El usuario no tiene una agencia asignada. No puede crear grupos.");
                return;
            }
    ...
    const handleUpdate = async (id: string | number, data: Record<string, any>) => {
        try {
            if (!userAgency) {
                showError("Error", "El usuario no tiene una agencia asignada. No puede actualizar grupos.");
                return;
            }
            ...
            showSuccess("¡Grupo Actualizado!", "El grupo ha sido actualizado exitosamente.");
            setEditingGroup(null);
            await fetchGroups();
        } catch (err: any) {
            showError("Error al Actualizar", err.message || "No se pudo actualizar el grupo.");
        }
    };
    ...
    const handleDelete = async (id: string | number) => {
        ...
        try {
            ...
            showSuccess("¡Grupo Eliminado!", "El grupo ha sido eliminado exitosamente.");
            setDeletingGroup(null);
            await fetchGroups();
        } catch (err: any) {
            showError("Error al Eliminar", err.message || "No se pudo eliminar el grupo.");
        }
    };
    ...
    const calculateYearsSinceDebut = (debutDate: Date | string) => {
        ...
        const debut = typeof debutDate === 'string' ? new Date(debutDate) : debutDate;
        const today = new Date();
        let years = today.getFullYear() - debut.getFullYear();
        const monthDiff = today.getMonth() - debut.getMonth();
        if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < debut.getDate())) {
            years--;
        }
    ...
    const mapTraineesForModal = (): TraineeForModal[] => {
        return apprentices.map((apprentice: ApprenticeResponseDto) => {
            ...
            return {
                ...
                lastName: apprentice.fullName.split(' ').slice(1).join(' ') || apprentice.fullName,
    ...
    const handleAddMemberToGroup = async (traineeId: string, role: string, endDate?: Date) => {
        ...
        try {
            ...
            if (existingArtist) {
                ...
                showSuccess("Miembro agregado", "El miembro ha sido agregado al grupo exitosamente.");
                ...
                setSelectedTraineeForArtist({
                    trainee: {
                        ...
                        lastName: trainee.fullName.split(' ').slice(1).join(' ') || trainee.fullName,
            ...
            showError("Error", err.message || "No se pudo agregar el miembro al grupo.");
        }
    };

    const handleCreateArtistAndAddToGroup = async (artistData: Record<string, any>) => {
        ...
        try {
            ...
            showSuccess("Artista creado y agregado", "El artista ha sido creado y agregado al grupo exitosamente.");
            setShowCreateArtistModal(false);
            setSelectedTraineeForArtist(null);
            await Promise.all([fetchGroups(), fetchArtists()]); // Refrescar datos
        } catch (err: any) {
            showError("Error", err.message || "No se pudo crear el artista y agregarlo al grupo.");
        }
        };
    ...
    const columns: Column<GroupResponseDto>[] = [
        {
            ...
            render: (item) => (
                <div className="group-name-cell">
                    ...
                    <div className="group-concept">{item.concept.substring(0, 50)}...</div>
                </div>
            )
        },
        {
        ...
        },
        {
            ...
            align: "center",
            render: (item) => (
                <span className={`members-count ${item.members_num > 0 ? 'has-members' : 'no-members'}`}>
                    {item.members_num}
                </span>
            )
        },
        {
            ...
            align: "center",
            render: (item) => (
                <span className={`created-badge ${item.is_created ? 'created' : 'not-created'}`}>
    ...
    const renderGroupDetails = (group: GroupResponseDto) => {
        return (
            <div className="group-details">
                ...
                <div className="detail-item">
                    <strong>Fecha de Debut:</strong> <span>{formatDate(group.debut_date)}</span>
                </div>
                <div className="detail-item">
                    <strong>Años Activos:</strong> <span>{calculateYearsSinceDebut(group.debut_date)}</span>
                </div>
                <div className="detail-item">
                    <strong>Número de Miembros:</strong> <span>{group.members_num}</span>
                </div>
                <div className="detail-item">
                    <strong>Grupo Creado:</strong> <span>{group.is_created ? "Sí" : "No"}</span>
                </div>
                <div className="detail-item">
        ...
        <section id="group_management" className="content-section active">
            ...
            {userAgency && (
                <div className="agency-info-banner">
                    <p>
                        <strong>Agencia Actual:</strong> {userAgency.nameAgency} - {userAgency.place}
                        <br />
                        <small>Todos los grupos creados o editados se asociarán automáticamente a esta agencia.</small>
                    </p>
                </div>
            )}
                ...
                <ViewMembersModal
                    ...
                    onRemoveMember={async (artistId) => {
                        try {
                            ...
                            showSuccess("Miembro removido", "El miembro ha sido removido del grupo exitosamente.");
                            setViewingGroup(null);
                        } catch (err: any) {
                            showError("Error", err.message || "No se pudo remover el miembro del grupo.");
                        }
                    }}
                />
            )}
