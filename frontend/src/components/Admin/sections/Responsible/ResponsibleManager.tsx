...
const ResponsibleManagement: React.FC = () => {
  ...
  const [editingResponsible, setEditingResponsible] = useState<ResponsibleResponseDto | null>(null);
  const [deletingResponsible, setDeletingResponsible] = useState<ResponsibleResponseDto | null>(null);
  ...
  const showCreateSuccess = () => {
    showSuccess("¡Responsable Creado!", "El responsable ha sido creado exitosamente.");
  };

  const showCreateError = (errorMessage?: string) => {
    showError("Error al Crear", errorMessage || "No se pudo crear el responsable.");
  };

  const showUpdateSuccess = () => {
    showSuccess("¡Responsable Actualizado!", "El responsable ha sido actualizado exitosamente.");
  };

  const showUpdateError = (errorMessage?: string) => {
    showError("Error al Actualizar", errorMessage || "No se pudo actualizar el responsable.");
  };

  const showDeleteSuccess = () => {
    showSuccess("¡Responsable Eliminado!", "El responsable ha sido eliminado exitosamente.");
  };

  const showDeleteError = (errorMessage?: string) => {
    showError("Error al Eliminar", errorMessage || "No se pudo eliminar el responsable.");
  };
  ...
  const responsibleFields: FormField[] = [
    {
      ...
      validate: (value) => {
        ...
        if (words.length < 2) return "Debe incluir al menos un nombre y un apellido";
